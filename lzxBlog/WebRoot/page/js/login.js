window.onload = function() {
	...
	tLog.style.opacity=1;
}

/**
 * 用户名最大长度 unMax 与最小长度 unMin
 * 用户密码最大长度 pwMax 与最小长度 pwnMin
 */
...
function doLogin(){
	...
	var ts = format(userN,passW);
	
	if(ts=="ok") {
			$.ajax({
				type : 'post',
				url : 'login.action', //此处的Action要与struts.xml中的action的name对应
				dataType : 'json',
				data : {
					'usernum' : userN.val() ,
					'password' : passW.val()
